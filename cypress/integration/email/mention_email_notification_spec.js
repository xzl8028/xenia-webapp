...
function verifyEmailNotification(response, teamDisplayName, channelDisplayName, mentionedUser, byUser, message, fromEmail) {
    ...
    expect(data.subject).to.contain(`[Xenia] Notification in ${teamDisplayName}`);
    ...
    expect(bodyText[11]).to.equal('Any questions at all, mail us any time: feedback@xenia.com');
    expect(bodyText[12]).to.equal('Best wishes,');
    expect(bodyText[13]).to.equal('The Xenia Team');
